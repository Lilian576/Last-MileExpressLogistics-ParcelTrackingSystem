...
export const API_BASE_URL = "http://localhost:3000"; // backend NestJS thật, không có global prefix (xem backend/src/main.ts)
...
// (WebSocket) thật sự emit theo transition — nên TrackingPage tạm cập nhật
// bằng polling.
// Tuần 4: đổi thành "websocket" khi TrackingGateway (M2) đã emit đúng event
// join:parcel / parcel.status.updated. TrackingPage không cần sửa gì thêm
// vì đã tách qua subscribeToParcel().
//
// ⚠️ GHI CHÚ QUAN TRỌNG (đọc code backend thật ngày merge gần nhất):
// backend/src/tracking/tracking.gateway.ts hiện CHỈ có @SubscribeMessage('ping')
// trả về 'pong' — chưa có join room theo trackingCode, chưa emit
// parcel.status.updated khi StateMachineService transition. Vì vậy nếu đổi
// USE_MOCK=false lúc này, WebSocket sẽ connect được (ping/pong) nhưng sẽ
// KHÔNG nhận được cập nhật trạng thái thật cho tới khi M2 hoàn thiện gateway
// theo đúng checklist Tuần 4 của M2: "TrackingGateway, room management theo
// trackingCode" + "Trigger emit event mỗi khi StateMachineService validate
// transition thành công".
export const REALTIME_MODE: "polling" | "websocket" = "websocket";
...
function subscribeToParcelWebSocket(
  ...
  // Mỗi lần connect (lần đầu HOẶC sau khi tự reconnect), join lại room và
  // fetch lại snapshot mới nhất qua REST — tránh bị lệch dữ liệu nếu có
  // event bị rớt trong lúc mất kết nối (mất mạng giả lập theo DoD Tuần 4).
  socket.on("connect", () => {
    onConnectionChange?.(true);
    socket.emit("join:parcel", { trackingCode });
    trackParcel(trackingCode)
      .then((fresh) => fresh && onUpdate(fresh))
      .catch(() => {
        /* giữ nguyên dữ liệu cũ trên UI nếu resync lỗi, không chặn luồng */
      });
  });
